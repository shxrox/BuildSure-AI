...
function BlueprintPage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      setUploading(true);
      await uploadBlueprint(id, file);
      await fetchBlueprintData();
      setFile(null);
      setMessage("Blueprint or JSON plan file uploaded successfully.");
  ...
  const fileUrl = blueprint?.fileName ? `/api/projects/${id}/blueprint` : null;

  return (
    <div className="p-8 max-w-4xl mx-auto font-sans">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">🖼 Architectural Blueprint & Plan File</h2>
          <p className="text-gray-600 text-sm mt-1">
            Upload and view original PDF, images, or exported JSON floor plan files for reference.
          </p>
        </div>
        <button
          onClick={() => navigate(`/projects/${id}/floor-plan`)}
          className="px-5 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 cursor-pointer transition-colors shadow-xs"
        >
          Open Floor Plan CAD ➔
        </button>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-xs border border-gray-200 mb-8">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Upload New Blueprint or JSON File</h3>
      ...
      <div className="bg-white p-6 rounded-xl shadow-xs border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Current Uploaded File</h3>
        {blueprint && blueprint.fileName ? (
          <div className="space-y-4">
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden flex justify-center bg-gray-100 p-4 min-h-[300px] items-center">
              {fileUrl && blueprint.fileName.match(/\.(jpeg|jpg|png|gif|webp)$/i) ? (
                <img src={fileUrl} alt="Blueprint Preview" className="max-h-[400px] object-contain rounded" />
              ) : (
                <div className="text-center text-gray-600">
                  <p className="font-semibold">Document / JSON Plan Preview</p>
                  <p className="text-xs text-gray-500 mt-1">File '{blueprint.fileName}' is attached and stored successfully for project reference.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <p className="text-sm text-gray-500 py-8 text-center border-t border-gray-100">
            No blueprint or JSON file uploaded yet. Use the upload form above to attach your project documents.
          </p>
        )}
      </div>
    </div>
