...
function FloorPlanPage() {
   ...
   * IMPORTANT:
   *
   * Pan only happens while panRef.current.active
   * is true.
   *
   * Previously the code checked:
   *
   * tool === "pan" || panRef.current.active
   *
   * which caused the canvas to move simply
   * when the mouse moved while the Pan tool
   * was selected.
   */
  const panRef = useRef<{
    active: boolean;
   ...
   * FINISH ROOM
   *
   * Right click uses this function.
   */
   ...
   * FINISH WALL DRAWING
   *
   * Right click stops the current wall chain.
   */
   ...
   * RIGHT CLICK
   *
   * This prevents the browser's default
   * context menu from appearing.
   *
   * Instead:
   *
   * WALL -> stop drawing
   * ROOM -> create/finish room
   */
  ...
  const handleMouseDown = async (
    ...
    /*
     * Ignore right click here.
     *
     * Right click is handled separately
     * by handleContextMenu().
     */
    if (e.button === 2) {
      return;
    }
     ...
     *
     * IMPORTANT:
     * We only activate panning on mouse down.
     *
     * Moving the mouse while the hand tool
     * is selected does NOTHING until the
    ...
    if (
       ...
       * Add another point.
       *
       * If clicking close to the
       * first point, finish room.
       */
      if (
        roomPoints.length >= 3 &&
        dist(
  ...
  const handleMouseMove = (
     ...
     * here.
     *
     * The canvas should only move when
     * panRef.current.active is true.
     */
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <canvas
            ...
            /*
             * THIS IS THE RIGHT-CLICK FIX.
             *
             * Browser context menu is prevented.
             * Our custom wall/room finish logic
             * runs instead.
             */
            onContextMenu={
              handleContextMenu
